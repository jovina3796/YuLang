...
export default async function FuelPage({
  ...
  searchParams: Promise<{ sort?: string; dir?: string; from?: string; to?: string; vehicle?: string; payment?: string }>
}) {
  const supabase = createServiceClient()
  const { sort, dir, from, to, vehicle, payment } = await searchParams
  ...
  // Query respects from/to/vehicle but NOT payment — so sidebar totals + the
  // payment-method dropdown always see the full set under the current date /
  // vehicle filter. Payment filter is applied client-side below.
  let q = supabase
    .from('fuel_logs')
  ...
  const allLogs = logs ?? []
  // Sidebar payment-method totals respect date/vehicle (already in query) but
  // ignore payment filter so user can see all options and switch between them.
  const paymentTotals = new Map<string, { count: number; total: number }>()
  for (const l of allLogs as any[]) {
    const key = (l.payment_method ?? '未指定') || '未指定'
    const prev = paymentTotals.get(key) ?? { count: 0, total: 0 }
    prev.count += 1
    prev.total += Number(l.total_cost ?? 0)
    paymentTotals.set(key, prev)
  }
  const paymentSummary = Array.from(paymentTotals.entries())
    .map(([method, v]) => ({ method, ...v }))
    .sort((a, b) => b.total - a.total)
  const paymentOptions = paymentSummary.map(p => p.method).filter(m => m !== '未指定')

  const displayedLogs = payment
    ? allLogs.filter((l: any) => (l.payment_method ?? '') === payment)
    : allLogs
  ...
  const hasFilter = !!(from || to || vehicle || payment)
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        <FuelDateFilter vehicles={vehicles ?? []} paymentMethods={paymentOptions} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '7fr 3fr', gap: 14, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div className="card" style={{ padding: '12px 14px' }}>
            <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 4 }}>付款方式金額統計</div>
            <div style={{ fontSize: 10, color: 'var(--text3)' }}>
              {hasFilter ? '依目前日期 / 車輛條件' : '最近 500 筆'}
            </div>
          </div>
          {paymentSummary.length === 0 ? (
            <div className="card" style={{ padding: 24, textAlign: 'center', color: 'var(--text3)', fontSize: 12 }}>尚無資料</div>
          ) : paymentSummary.map(p => {
            const isActive = payment === p.method
            return (
              <div key={p.method} className="card" style={{
                padding: '12px 14px',
                borderColor: isActive ? 'var(--amber2)' : undefined,
                borderWidth: isActive ? 2 : undefined,
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                  <div style={{ fontSize: 12, color: 'var(--text2)', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.method}</div>
                  <div style={{ fontSize: 10, color: 'var(--text3)', flexShrink: 0, marginLeft: 8 }}>{p.count} 筆</div>
                </div>
                <div className="mono" style={{ fontSize: 18, color: 'var(--amber2)', fontWeight: 700, textAlign: 'right' }}>
