...
import { importFuelCsv } from '@/app/(dashboard)/fuel/import-actions'

export default function FuelImportExport() {
  ...
  const exportHref = `/api/fuel/export?${searchParams.toString()}`
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res  = await importFuelCsv(text)
      ...
      {open && (
        ...
        >
          <div style={{
            ...
            <div style={{ fontSize: 15, fontWeight: 700 }}>批次上傳加油紀錄（CSV）</div>

            <div style={{ fontSize: 12, color: 'var(--text3)', lineHeight: 1.6 }}>
              <div>必要欄位：<b>日期、車號、金額</b></div>
              <div>選填欄位：目前里程、付款方式、備註</div>
