...
import {
  createLeave, updateLeave, uploadLeaveReceipt,
  type LeaveInput, type LeaveType,
...
const TYPES: { v: LeaveType; label: string }[] = [
  { v: 'sick',     label: '病假' },
  { v: 'personal', label: '事假' },
  { v: 'annual',   label: '特休' },
  { v: 'other',    label: '其他' },
...
export default function LeaveFormModal({ drivers, mode, initial }: Props) {
  ...
  const [leaveType,  setLeaveType]  = useState<LeaveType>(initial?.leave_type ?? 'personal')
  ...
  function reset() {
    if (mode === 'create') {
      setDriverId(''); setLeaveType('personal'); setStartDate(today); setEndDate(today)
      setHours(''); setReason(''); setNotes('')
  ...
  async function handleSubmit() {
    ...
    if (receiptFile) {
      ...
      const up = await uploadLeaveReceipt(fd)
  ...
  const trigger = mode === 'create'
    ? <button className="btn btn-primary" onClick={() => setOpen(true)} title="新增請假"
      ...
      {open && (
        ...
        >
          <div style={{
            ...
            <div style={{ fontSize: 15, fontWeight: 700 }}>{mode === 'create' ? '新增請假' : '編輯請假'}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
              <label style={L}>
                <span style={LT}>起始日期</span>
                <input type="date" className="input" value={startDate} onChange={e => setStartDate(e.target.value)} />
              </label>
              <label style={L}>
                <span style={LT}>結束日期</span>
                <input type="date" className="input" value={endDate} onChange={e => setEndDate(e.target.value)} />
              </label>
              <label style={L}>
                <span style={LT}>時數（選填）</span>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 4 }}>
              ...
              <button className="btn btn-primary"
                      disabled={!driverId || !startDate || !endDate || saving}
