...
import { importTripsCsv } from '@/app/(dashboard)/trips/import-actions'

export default function TripImportExport() {
  ...
  const exportHref = `/api/trips/export?${searchParams.toString()}`
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res  = await importTripsCsv(text)
      ...
      {open && (
        ...
        >
          <div style={{
            ...
            <div style={{ fontSize: 15, fontWeight: 700 }}>批次上傳車趟（CSV）</div>

            <div style={{ fontSize: 12, color: 'var(--text3)', lineHeight: 1.6 }}>
              <div>必要欄位：<b>日期、廠商、業務類別、趟數</b></div>
              <div>選填欄位：倉庫、地區、司機、車號、配送點數、配送區域備註、<b>運費</b>（留空自動依運費規則計算）、KPI達標、加成費、備註</div>
